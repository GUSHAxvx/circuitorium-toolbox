...
想让全班用手机打开？（同一个 WiFi 就行）
--------------------------------------
双击「启动（全班可访问）.bat」→ 窗口里会打印几个网址，挑和手机同网段的那个念给全班。
- 学生不用装任何东西，浏览器输网址即可；内容是只读的，别人改不了你的数据
- 第一次运行如果弹出防火墙提示，选「允许访问」；还是打不开就按窗口里的提示放行 8734 端口
- 手机和电脑必须连同一个 WiFi（用手机流量打不开）
- 只是自己用的话，还是双击「启动.bat」，那样只有本机能访问，更安全
...
--------
启动.bat              双击这个（只有本机能访问）
启动（全班可访问）.bat  同一个 WiFi 下的手机/别的电脑也能打开
