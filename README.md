...
服务器版的顶栏第一个入口就是「工具箱」，元件库、始解/卍解、作品文件传播这些和本地版一样都能用
（卍解的程序代码/接线表/调试记录在服务器端存在 SQLite 里，界面同一套）。

> **关于服务器版的定位**：这个项目是个人开发的，撑不起服务器成本，所以重心放在两个本地版上；
> 服务器版保留、也照样能用，但**「云端共享」这个方向暂时停止推进**（详见根目录的 `服务器版说明.txt`）。
